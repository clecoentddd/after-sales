...
export class OnHoldQuotationAggregate {
  ...
  static putOnHold(command, currentQuotationState) {
    console.log(`[OnHoldQuotationAggregate] Attempting to put quotation ${command.quoteId} on hold.`);

    if (!currentQuotationState) {
      console.warn(`[OnHoldQuotationAggregate] Cannot put quotation ${command.quoteId} on hold: Quotation state not found.`);
      return null;
    }
    ...
    if (currentQuotationState.status === 'Draft' || currentQuotationState.status === 'Pending') {
      console.log(`[OnHoldQuotationAggregate] Quotation ${command.quoteId} is in status '${currentQuotationState.status}'. Emitting QuotationOnHoldEvent.`);
      ...
      console.warn(`[OnHoldQuotationAggregate] Cannot put quotation ${command.quoteId} on hold: Current status is '${currentQuotationState.status}'. Only 'Draft' or 'Pending' can be put on hold.`);

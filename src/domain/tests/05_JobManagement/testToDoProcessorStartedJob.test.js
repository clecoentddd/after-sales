...
describe('Change Request Assessment Workflow', () => {
  ...
  it('should create and process a change request todo after job is started', async () => {
    console.log('\n=== Starting change request test with job started ===');
    ...
    await new Promise((resolve) => {
      const checkInterval = setInterval(async () => {  // Added async here
        ...
        if (toDoDB.length === 1 && toDoDB[0].flag === 'processed') {
          ...
          console.log('[TEST] Final Job Aggregate State:');
          console.log('----------------------------------');
          ...
          // Verify CRstatus
          expect(jobAggregate.CRstatus).toBe('ChangeRequestReceivedPendingAssessment');
          resolve();
        }
      }, 100);

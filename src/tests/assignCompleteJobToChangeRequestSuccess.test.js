...
describe('Assign Job to Change Request - Success Scenario', () => {
  ...
  it('should update the todo list with the change request for assessment when a job exists', (done) => {
    ...
    eventBus.subscribe('CompleteJobAssignedToChangeRequest', (event) => {
       try {
        console.log('[Test] Received CompleteJobAssignedToChangeRequest event:', event);

        // Verify the job assignment details
        expect(event.type).toBe('CompleteJobAssignedToChangeRequest');

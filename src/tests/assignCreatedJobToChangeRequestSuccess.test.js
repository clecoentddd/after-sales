...
import { initializeAssignCreatedJobToChangeRequestProcessor } from '../domain/features/05_JobManagement/23a_SetTodoToPutJobOnHold/initializeAssignCreatedJobToChangeRequestProcessor';
...
describe('Assign Job to Change Request - Success Scenario', () => {
  ...
  it('should update the todo list with the change request for assessment when a job exists', (done) => {
    ...
    eventBus.subscribe('CreatedJobAssignedToChangeRequest', (event) => {
       try {
        console.log('[Test] Received CreatedJobAssignedToChangeRequest event:', event);

        // Verify the job assignment details
        expect(event.type).toBe('CreatedJobAssignedToChangeRequest');
